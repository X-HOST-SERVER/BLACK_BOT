...
handler.all = async function (m) {
  ...
  let fake = {
    key: {
      fromMe: false,
      participant: '𝐁𝐋𝐀𝐂𝐊_𝐁𝐎𝐓@s.whatsapp.net',
      remoteJid: '120363384250924818@g.us',
    },
    message: {
      conversation: '｢🍷┊𝐁𝐋𝐀𝐂𝐊_𝐁𝐎𝐓┊🍭｣'
    },
  ...
  if (/^عبيط|يا عبيط|اهبل|غبي$/i.test(m.text)) { 
    conn.reply(m.chat, `*كفايه بقا كذب كلنا عارفين انك انت العبيط 😂*`, fake);
  }
  ...
  if (/^ببحبك بوت|حبك|بوت بحبك$/i.test(m.text)) { 
    conn.reply(m.chat, `اسكت بدل ما انادي بلاك يفشخك`, fake);
  }
